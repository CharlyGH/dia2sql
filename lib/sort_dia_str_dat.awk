function substring_after(strg, key,     idx) {
    idx = index(strg, key);
    if (idx == 0) {
        return "";
    }
    else {
        return substr(strg,idx+1);
    }
}
...
BEGIN {
    in_table_def = 0;
    foreign_count = 0;
    schema_sort_order["Base"] = 0;
    schema_sort_order["Dim"]  = 1;
    schema_sort_order["Fact"] = 3;
    schema_sort_order["Hist"] = 2;
...
/^schema#/ {
    ...
    for (idx = 1; idx <= cnt; idx++) {
        ...
        table_sort_order[table_name] = schema_sort_order[schema_code];
        if (schema_code != "Hist") {
...
/^table#/ {
    ...
    order = table_sort_order[table];
    full_table_name = schema "#" table;
    full_name_by_id_tab[id] = full_table_name;
    table_info_tab[full_table_name] = $0;
    # use prefix order because asorti in /end/ does not work as expected
    sort_table_info_tab[order "#" full_table_name] = order "#" $0;
    col_cnt = 0;
    in_table_def = 1;
}
...
/^end/ {
    ...
    if ((section == "metadata") || (section == "domains") || (section == "sequences")) {
        ...
        if (! in_table_def) {
            tab_cnt = asorti(sort_table_info_tab, table_srt_tab);
            for (tab_idx = 1; tab_idx <= tab_cnt; tab_idx++) {
                full_table_name = table_srt_tab[tab_idx];
                # cut order from full_table_name, see /table/
                full_table_name = substring_after(full_table_name, "#")
        ...
        for (fk_idx = 1; fk_idx <= fk_cnt; fk_idx++) {
            print fk_tab[fk_srt_tab[fk_idx]] "#-----#" fk_srt_tab[fk_idx];
        }
        print "end";
    }
    else {
