...
<xsl:stylesheet version="1.0"
  ...
  <!--
      ...
      #   2. const (dimension) tables
      #      2.1 first column should be a primary key of one id_type
      #      2.2 it should have a unique key of one text_type
      #   3. dim (dimension) tables
      #      3.1 first column should be a primary key of one id_type
      #      3.2 it should have a unique key of one text_type
      #      3.3 it should have a valid_from field if historisation is active
  ...
  <xsl:template match="model">
    <xsl:variable name="errors">
      ...
      <xsl:apply-templates select="tables/table[@schema = $const-schema]" mode="rule_2_1"/>
      <xsl:apply-templates select="tables/table[@schema = $const-schema]" mode="rule_2_2"/>

      <xsl:apply-templates select="tables/table[@schema = $dim-schema]" mode="rule_3_1"/>
      <xsl:apply-templates select="tables/table[@schema = $dim-schema]" mode="rule_3_2"/>
      <xsl:apply-templates select="tables/table[@schema = $dim-schema]" mode="rule_3_3"/>

      <xsl:apply-templates select="tables/table[@schema = $fact-schema]" mode="rule_4_1"/>
      <xsl:apply-templates select="tables/table[@schema = $fact-schema]" mode="rule_4_2"/>
      <xsl:apply-templates select="tables/table[@schema = $fact-schema]" mode="rule_4_3"/>
      
      <xsl:if test="$historisation = 'true'">
        <xsl:apply-templates select="tables/table[@schema = $hist-schema]" mode="rule_5_1"/>
  ...
  <xsl:template match="primary" mode="rule_3_1">
    ...
    <xsl:if test="$primary-count != 1">
      <xsl:value-of select="concat('rule_3_1',$d,$primary-count,$nl)"/>
    </xsl:if>
    <xsl:value-of select="$key-errors"/>
  </xsl:template>
  ...
  <xsl:template match="key" mode="rule_3_1">
    ...
    <xsl:variable name="wrong-position" select="concat('primary key ',$column,' is not first column')"/>
    <xsl:if test="$type != 'id_type'">
      <xsl:value-of select="concat('rule_3_1:ERROR:',$schema,$s,$table,$s,$column,$d,$primary,$d,$wrong-type,$nl)"/>
    </xsl:if>
    <xsl:variable name="first-col"
                  select="//table[@name = $table and @schema = $schema]//column[position() = 1]/@name"/>
    <xsl:if test="$first-col != $column">
      <xsl:value-of select="concat('rule_3_1:ERROR:',$schema,$s,$table,$s,$column,$d,$first-col,$d,$wrong-position,$nl)"/>
    </xsl:if>
  ...
  <xsl:template match="table" mode="rule_3_2">
    ...
    <xsl:variable name="missing-unique" select="'no unique key'"/>
    <xsl:variable name="errors">
      <xsl:apply-templates select="unique"  mode="rule_3_2">
        <xsl:with-param name="schema" select="$schema"/>
        <xsl:with-param name="table" select="$table"/>
      </xsl:apply-templates>
    </xsl:variable>
    <xsl:choose>
      <xsl:when test="$unique-count = 0">
        <xsl:value-of select="concat('rule_3_2:WARNING:',$schema,$s,$table,$d,$missing-unique,$nl)"/>
      </xsl:when>
      <xsl:when test="string-length($errors) != 0">
        <xsl:value-of select="$errors"/>
      </xsl:when>
  ...
  <xsl:template match="unique" mode="rule_3_2">
    <xsl:param name="table"/>
    <xsl:param name="schema"/>
    <xsl:variable name="unique" select="@name"/>
    <xsl:variable name="unique-count" select="count(key)"/>
    <xsl:variable name="key-errors">
      <xsl:apply-templates select="key"  mode="rule_3_2">
        <xsl:with-param name="schema" select="$schema"/>
        <xsl:with-param name="table" select="$table"/>
        <xsl:with-param name="unique" select="$unique"/>
      </xsl:apply-templates>
    ...
    <xsl:if test="$unique-count = 0">
      <xsl:value-of select="concat('rule_3_2:ERROR:',$d,$schema,$d,$table,$d,$unique-count,$nl)"/>
    </xsl:if>
    <xsl:value-of select="$key-errors"/>
  </xsl:template>


  <xsl:template match="key" mode="rule_3_2">
    <xsl:param name="table"/>
    <xsl:param name="schema"/>
    <xsl:param name="unique"/>
    <xsl:variable name="key" select="text()"/>
    ...
    <xsl:if test="$type != 'text_type' and $type != 'date_type'">
      <xsl:value-of select="concat('rule_3_2:ERROR:',$schema,$d,$table,$d,$key,$d,$unique,$d,$type,$nl)"/>
    </xsl:if>
  </xsl:template>
      ...
      <xsl:when test="$historisation = 'true'">
        <xsl:choose>
          <xsl:when test="$column-count != 1">
            <xsl:value-of select="concat('rule_3_3:ERROR:',$d,$schema,$s,$table,$d,$column-count,$missing,$nl)"/>
          </xsl:when>
          <xsl:otherwise>
            <xsl:value-of select="concat('rule_3_3:OK:',$d,$schema,$s,$table,$d,$column-count,$nl)"/>
          </xsl:otherwise>
        </xsl:choose>
      </xsl:when>
      <xsl:otherwise>
        <xsl:choose>
          <xsl:when test="$column-count = 1">
            <xsl:value-of select="concat('rule_3_3:ERROR:',$d,$schema,$s,$table,$d,$column-count,$unexpected,$nl)"/>
          </xsl:when>
          <xsl:otherwise>
            <xsl:value-of select="concat('rule_3_3:OK:',$d,$schema,$s,$table,$d,$column-count,$nl)"/>
          </xsl:otherwise>
  ...
  <xsl:template match="table" mode="rule_4_1">
    ...
    <xsl:choose>
      <xsl:when test="$primary-count = 0">
        <xsl:value-of select="concat('rule_4_1:OK:',$schema,$s,$table,$d,$primary-count,$nl)"/>
      </xsl:when>
      <xsl:when test="$errors != ''">
        <xsl:value-of select="$errors"/>
      </xsl:when>
      <xsl:otherwise>
        <xsl:value-of select="concat('rule_4_1:OK:',$schema,$s,$table,$nl)"/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>
  ...
  <xsl:template match="primary" mode="rule_4_1">
    ...
    <xsl:variable name="key-errors">
      <xsl:apply-templates select="key"  mode="rule_4_1">
        <xsl:with-param name="schema" select="$schema"/>
        <xsl:with-param name="table" select="$table"/>
        <xsl:with-param name="primary" select="$primary"/>
      </xsl:apply-templates>
    ...
    <xsl:if test="$id-primary-count = 0">
      <xsl:value-of select="concat('rule_4_1:ERROR:',$schema,$d,$table,$d,$id-error,$nl)"/>
    </xsl:if>
    <xsl:if test="$date-primary-count &gt; 1">
      <xsl:value-of select="concat('rule_4_1:ERROR:',$schema,$d,$table,$d,$date-error,$nl)"/>
    </xsl:if>
    <xsl:value-of select="$key-errors"/>
  </xsl:template>


  <xsl:template match="key" mode="rule_4_1">
    <xsl:param name="table"/>
    <xsl:param name="schema"/>
    <xsl:param name="primary"/>
    <xsl:variable name="key" select="text()"/>
    ...
    <xsl:if test="$type != 'id_type' and $type != 'date_type'">
      <xsl:value-of select="concat('rule_4_1:ERROR:',$schema,$d,$table,$d,$primary,$d,$type,$nl)"/>
    </xsl:if>
  </xsl:template>
  ...
  <xsl:template match="table" mode="rule_4_2">
    ...
    <xsl:choose>
      <xsl:when test="$unique-count != 0">
        <xsl:value-of select="concat('rule_4_2:WARNING:',$schema,$d,$table,$d,$unique-error,$nl)"/>
      </xsl:when>
      <xsl:otherwise>
        <xsl:value-of select="concat('rule_4_2:OK:',$schema,$s,$table,$nl)"/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>
  ...
  <xsl:template match="table" mode="rule_4_3">
    ...
    <xsl:choose>
      <xsl:when test="$primary-count != 1">
        <xsl:value-of select="concat('rule_4_3:ERROR:',$schema,$s,$table,$d,$primary-count,$nl)"/>
      </xsl:when>
      <xsl:when test="$errors != ''">
        <xsl:value-of select="$errors"/>
      </xsl:when>
      <xsl:otherwise>
        <xsl:value-of select="concat('rule_4_3:OK:',$schema,$s,$table,$nl)"/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>


  <xsl:template match="primary" mode="rule_4_3">
    <xsl:param name="table"/>
    <xsl:param name="schema"/>
    <xsl:variable name="primary" select="@name"/>
    <xsl:variable name="errors">
      <xsl:apply-templates select="key"  mode="rule_4_3">
        <xsl:with-param name="schema" select="$schema"/>
        <xsl:with-param name="table" select="$table"/>
        <xsl:with-param name="primary" select="$primary"/>
      </xsl:apply-templates>
  ...
  <xsl:template match="key" mode="rule_4_3">
    <xsl:param name="table"/>
    <xsl:param name="schema"/>
    <xsl:param name="unique"/>
    <xsl:variable name="column" select="text()"/>
    ...
    <xsl:if test="$type = 'id_type'">
      ...
      <xsl:if test="not(contains($ref-pk,$column))">
        <xsl:value-of select="concat('rule_4_3:WARNING:',$schema,$d,$table,$s,$column,$d,$pk-error,$nl)"/>
      </xsl:if>
    </xsl:if>    
  </xsl:template>


  <xsl:template match="table" mode="rule_5_1">
    <xsl:variable name="table" select="@name" />
    <xsl:variable name="schema" select="@schema"/>
    <xsl:variable name="primary-count" select="count(primary)"/>
    ...
    <xsl:choose>
      <xsl:when test="$primary-count != 1 or $primary-key-count != 2">
        ...
        <xsl:value-of select="concat('rule_5_1:ERROR:',$schema,$d,$table,$s,$column,$d,$pk-error,$nl)"/>
      </xsl:when>
      <xsl:otherwise>
        <xsl:variable name="errors">
          <xsl:apply-templates select="primary"  mode="rule_5_1">
            <xsl:with-param name="schema" select="$schema"/>
            <xsl:with-param name="table" select="$table"/>
          </xsl:apply-templates>
        </xsl:variable>
        <xsl:choose>
          ...
          <xsl:otherwise>
            <xsl:value-of select="concat('rule_5_1:OK:',$schema,$d,$table,$nl)"/>
          </xsl:otherwise>
        </xsl:choose>
      </xsl:otherwise>
    </xsl:choose>
  ...
  <xsl:template match="primary" mode="rule_5_1">
    <xsl:param name="table"/>
    <xsl:param name="schema"/>
    ...
    <xsl:if test="$type-1 != 'date-type'">
      <xsl:value-of select="concat('rule_5_1:ERROR:',$schema,$s,$table,$s,$column-1,$error-1,$nl)"/>
    </xsl:if>
  ...
  <xsl:template match="table" mode="rule_5_2">
    <xsl:variable name="table" select="@name" />
    <xsl:variable name="schema" select="@schema"/>
    ...
    <xsl:choose>
      <xsl:when test="$unique-count != 0">
        <xsl:value-of select="concat('rule_5_2:ERROR:',$schema,$s,$table,$d,$unique-error,$nl)"/>
      </xsl:when>
      <xsl:otherwise>
        <xsl:value-of select="concat('rule_5_2:OK:',$schema,$d,$table,$nl)"/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>

  
  <xsl:template match="table" mode="rule_5_3">
    <xsl:variable name="table" select="@name" />
    <xsl:variable name="schema" select="@schema"/>
    ...
    <xsl:choose>
      <xsl:when test="$valid-to-count != 1">
        ...
        <xsl:value-of select="concat('rule_5_4:ERROR:',$schema,$s,$table,$d,$count-error,$nl)"/>
      </xsl:when>
      <xsl:otherwise>
        <xsl:variable name="errors">
        ...
        <xsl:choose>
          ...
          <xsl:otherwise>
            <xsl:value-of select="concat('rule_5_4:OK:',$schema,$s,$table,$nl)"/>
          </xsl:otherwise>
        </xsl:choose>
      </xsl:otherwise>
    </xsl:choose>
  ...
  <xsl:template match="column" mode="rule_5_3">
    <xsl:param name="table"/>
    <xsl:param name="schema"/>
    <xsl:variable name="column" select="@name"/>
    <xsl:variable name="type" select="@type"/>
    <xsl:if test="$type != 'date_type'">
      ...
      <xsl:value-of select="concat('rule_5_4:ERROR:',$schema,$s,$table,$d,$key-error,$nl)"/>
    </xsl:if>
  </xsl:template>
