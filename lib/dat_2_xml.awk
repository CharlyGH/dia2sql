...
/^schema#/ {
    ...
    for (idx = 1; idx <= cnt; idx++) {
        table = table_tab[idx];
        if (kind != "hist") {
            table_kind_tab[table] = kind;
            table_schema_tab[table] = schema_name;
        }
        if (kind == "hist") {
            table_hist_tab[table] = kind;
        }
    }
}
...
/^sequence#/ {
    ...
    sequence_name     = "seq_" sequence_column;
...
/^table#/ {
    ...
    schema_kind      = tolower($4);
    schema_name      = schema_name_tab[schema_kind];
    table_comment    = $5;
    column_count     = 0;
...
/^end/ {
    if (section == "metadata") {
        ...
        hist        = table_hist_tab[table_name];
        kind        = table_kind_tab[table_name];

        is_dim_table = (table_schema == dim_schema);
        table_definition = "name='" table_name "' " ;
        table_definition = table_definition "schema='" schema_name "' ";
        table_definition = table_definition "tablespace='" schema_name "' ";
