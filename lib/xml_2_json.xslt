...
<xsl:stylesheet version="1.0"
  ...
  <fcn:function name="fcn:get-table-kind">
    <xsl:param name="schema"/>
    <xsl:variable name="table-kind">
      <xsl:choose>
        <xsl:when test="$schema = $const-schema">
          <xsl:value-of select="'const'"/>
        </xsl:when>
        <xsl:when test="$schema = $dim-schema">
          <xsl:value-of select="'dimension'"/>
        </xsl:when>
        <xsl:when test="$schema = $fact-schema">
          <xsl:value-of select="'fact'"/>
        </xsl:when>
        <xsl:when test="$schema = $hist-schema">
          <xsl:value-of select="'history'" />
        </xsl:when>
        <xsl:otherwise>
          <xsl:message terminate="yes">
            <xsl:value-of select="concat('const: [',$const-schema,']',$nl)"/>
            <xsl:value-of select="concat('Unknown schema prefix in ',$schema,$nl)"/>
          </xsl:message>
        </xsl:otherwise>
      </xsl:choose>
    </xsl:variable>
    <fcn:result>
      <xsl:value-of select="$table-kind"/>
    </fcn:result>
  </fcn:function>
  ...
  <xsl:template match="model">
    ...
    <xsl:value-of select="concat($gtab,$dq,'model',$dq,': {',$nl)"/>
    <xsl:value-of select="concat($nltab,$dq,'project',$dq,': ',$dq,@project,$dq,',',$nl)"/>
    <xsl:value-of select="concat($nltab,$dq,'version',$dq,': ',    @version,        $nl)"/>
    <xsl:value-of select="concat($gtab,'},',$nl)"/>
  ...
  <xsl:template match="table" mode="detail">
    ...
    <xsl:variable name="has-fk"         select="fcn:if-then-else($foreign-count,'!=',0,'true','false')"/>
    <xsl:variable name="ref-count"      select="count(//reference/target[@schema = $schema and @table = $table])" />
    <xsl:variable name="delim"          select="fcn:get-delimiter(position(),last(),$comma)"/>
    <xsl:variable name="table-kind"     select="fcn:get-table-kind($schema)"/>
    ...
    <xsl:value-of select="concat($nltab,$dq,'has-fk',       $dq,': ',    $has-fk,           ',',$nl)"/>
  ...
  <xsl:template match="table" mode="reference">
    ...
    <xsl:value-of select="concat($nltab,$dq,'table', $dq,': ',$dq,$table  ,   $dq,',',$nl)"/>
    <xsl:value-of select="concat($nltab,$dq,'type',  $dq,': ',$dq,$table-kind,$dq,',',$nl)"/>
    ...
    <xsl:variable name="has-fk"         select="fcn:if-then-else($foreign-count,'!=',0,'true','false')"/>
    <xsl:variable name="opt-comma"      select="fcn:if-then-else($foreign-count,'!=',0,',','')"/>
    <xsl:value-of select="concat($nltab,$dq,'has-fk',       $dq,': ',    $has-fk,           $opt-comma,$nl)"/>
  ...
  <xsl:template match="column" mode="detail">
    ...
    <xsl:variable name="desc"        select="comment" />
    <xsl:variable name="isid"        select="fcn:if-then-else($raw-type,' = ','id_type','true','false')"/>
    ...
    <xsl:value-of select="concat($nltab,$dq,'isid',    $dq,': ',    $isid,        $comma,$nl)"/>
