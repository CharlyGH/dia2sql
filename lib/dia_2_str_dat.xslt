...
<xsl:stylesheet version="1.0"
  ...
  <fcn:function name="fcn:get-schema-name">
    ...
    <xsl:variable name="schema">
      <xsl:value-of select="document($schemainfo)/schmainfo/info[@table = $table]/@schema"/>
    </xsl:variable>
    <fcn:result>
  ...
  <xsl:template match="dia:object" mode="proc-metadata">
    ...
    <xsl:if test="$table-name = $proc-table-name">
      ...
      <xsl:value-of select="concat('end',$nl)" />
      <exslt:document href="{$schemainfo}" method="xml" omit-xml-declaration="no" indent="yes">
        <xsl:element name="schmainfo">
          <xsl:apply-templates select="dia:attribute[@name='attributes']/dia:composite" mode="metadata-xml" />
        </xsl:element>
      </exslt:document>
    </xsl:if>
  </xsl:template>
  ...
  <xsl:template match="dia:object" mode="proc-table">
    ...
    <xsl:variable name="table" select="fcn:strip(dia:attribute[@name = 'name']/dia:string/text())"/>
    <xsl:variable name="schema" select="fcn:get-schema-name($table)"/>
    <xsl:if test="not(contains($skip-table-name-list,$table))">
      <xsl:variable name="comment" select="fcn:strip(dia:attribute[@name='comment']/dia:string/text())" />
      <xsl:value-of select="concat('table#',@id,'#',$table,'#',$schema,'#',$comment,$nl)" />
  ...
  <xsl:template match="dia:composite" mode="metadata-xml" >
    <xsl:variable name="name" select="fcn:strip(dia:attribute[@name='name']/dia:string/text())" />
    <xsl:variable name="lc-name" select="fcn:to-lower-case($name)" />
    <xsl:variable name="type" select="fcn:strip(dia:attribute[@name='type']/dia:string/text())" />
    <xsl:variable name="comment" select="fcn:strip(dia:attribute[@name='comment']/dia:string/text())" />
    <xsl:if test="$lc-name = 'schema' and ($type = 'Const' or $type = 'Dim' or $type = 'Fact')">
      <xsl:for-each select="str:split($comment,',')"> 
  ...
  <xsl:template match="dia:composite" mode="sequence" >
    ...
    <xsl:if test="$pk = 'true' and $type = 'Id_Type' and ($schema = 'Dim' or $schema = 'Const')" >
