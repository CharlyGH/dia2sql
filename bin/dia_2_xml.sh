...
HELP="${USAGE}
    ...
    -i inputfile    name of input file --path "${DATA_DIR}"
...
DATA_MODEL_DTD="${DATA_DIR}/model.dtd"
TEMP_MODEL_DTD="${TEMP_DIR}/model.dtd"
...
[[ -z "${output}" ]] && output="${DATA_DIR}/${name}.xml"
...
[[ -n "${verbose}" ]] && echo "xsltproc ${xslt_params} ${DIA_2_STR_DAT} ${input} ${tempsd}"
xsltproc ${xslt_params} "${DIA_2_STR_DAT}" "${input}" > "${tempsd}"
ret="$?"
...
xslt_params="${xslt_params} --stringparam projectfile ${PROJECT_FILE}"
xslt_params="${xslt_params} --stringparam trigger ${trigger}"
xslt_params="${xslt_params} --path ${DATA_DIR}"
...
if [[ -n "${check}" ]] ; then
    ...
    [[ -n "${verbose}" ]] && \
        echo "${BIN_DIR}/create_config.sh ${verb_opt} -i ${tempout} -p ${projectfile} -o ${tempprj}"
    ...
    [[ -n "${verbose}" ]] && echo "${BIN_DIR}/check_rules.sh ${verb_opt} -i ${tempout} -p ${tempprj}" 
    ${BIN_DIR}/check_rules.sh ${verb_opt} -i "${tempout}"  -p "${tempprj}" 
    ret="$?"
...
if [[ -n "${auto}" ]] ; then
    ...
    [[ -n "${verbose}" ]] && echo "cat ${tempqt} ${output}"
    cat "${tempqt}" | tr '"' "'"  > "${output}"
    done="1"
    if [[ -n "${check}" ]] ; then
        [[ -n "${verbose}" ]] && echo "${BIN_DIR}/check_rules.sh ${verb_opt} -a -i ${output}" -p "${tempprj}" 
        "${BIN_DIR}/check_rules.sh" ${verb_opt} -a -i "${output}" -p "${tempprj}" 
        ret="$?"
        [[ "${ret}" != "0" ]] && error_exit "error in script check_rules.sh" "" 1
    fi
else
    [[ -n "${verbose}" ]] && echo "cp ${tempout} ${output}"
    cp "${tempout}" "${output}" 
fi


if [[ -z "${keep}" ]] ; then
    for file in "${tempsd}" "${tempss}" "${temppd}" "${tempchk}" "${tempout}" "${tempinfo}" ; do
        [[ -f "${file}" ]] && rm "${file}"
    done
fi
