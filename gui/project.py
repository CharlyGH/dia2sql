...
class Project(object):
    ...
    def get_instance(cls, filename=None):
        if cls.instance is None:
            ...
            cls.project_file = cls.root_dir + "/" + filename
            with open(cls.project_file) as stream:
    ...
    def get_config_list(self):
        config_list = list()
        for conf in self.data["config"].values():
    ...
    def get_model_info(self,key=None):
        ...
        if key == None:
            return model["project"], model["version"]
        else:
            return model[key]
    ...
    def get_alias(self, table_name):
        ...
        for name in name_list:
            alias = alias + name[0].lower()
        return alias
        

    def get_ref_list(self, reference_list, parent_alias, schema_list, table_list, key_list, unique_list, palias_list):
        for reference in reference_list:
            #print("reference=", reference)
            if not (reference["schema"] in schema_list and reference["table"] in table_list):
                schema = reference["schema"]
                table  = reference["table"]
                key    = reference["key"][0]
                ttype  = reference["type"]
                if ttype == "dim":
                    unique = reference["unique"]
                else:
                    unique = list()
                    column_list = self.get_column_list(schema, table)
                    for column in column_list:
                        column_name = column["name"]
                        isid = column["isid"]
                        isfk = column["isfk"]
                        if not isfk and not isid:
                            unique.append(column_name)
                palias_list.append(parent_alias)
                schema_list.append(schema)
                table_list.append(table)
                key_list.append(key)
                unique_list.append(unique)
                alias = self.get_alias(table)
            if reference["has-fk"]:
                self.get_ref_list(reference["reference"], alias, schema_list, table_list, key_list, unique_list, palias_list)
        return schema_list, table_list, key_list, unique_list, palias_list
    
    
    def get_reference_list(self, schema_name, table_name):
        print("Project.get_reference_list", schema_name, table_name)
        schema_list = list()
        table_list  = list()
        key_list    = list()
        unique_list = list()
        palias_list = list()
        alias = self.get_alias(table_name)
        table = self.data[schema_name][table_name]
        #print(alias)
        #print(table)
        
        if table["has-fk"]:
            return self.get_ref_list(table["reference"], alias, schema_list, table_list, key_list, unique_list, palias_list)
        else:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        filename = "out/verein_v01.json"        
    else:
        filename = sys.argv[1]

    project = Project.get_instance(filename)
    print (project.get_config_list())
#    print(project.to_xml())
#    print ("schema_list=", project.get_schema_list())
#    print ("table_list=", project.get_table_list("dim_firma"))
#    print ("names=", project.get_schema_info_list("name"))
#    print ("comments=", project.get_schema_info_list("comment"))
