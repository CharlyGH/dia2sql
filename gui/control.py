...
class Control(tk.Frame):
    def __init__(self, win, schema_name, table_name, data, **kwargs):
        ...
        bgcolor = config.get("control.bg.color")
        ...
        self.i_button = tk.Button(head, text="Einfügen", 
                                  command=ft.partial(self.execute_control_command,"insert"))
        ...
        self.u_button = tk.Button(head, text="Ändern", 
                                  command=ft.partial(self.execute_control_command,"update"))
        ...
        self.d_button = tk.Button(head, text="Löschen", 
                                  command=ft.partial(self.execute_control_command,"delete"))
        ...
        self.s_button = tk.Button(head, text="Suchen", 
                                  command=ft.partial(self.execute_control_query,"search"))
        ...
        self.c_button = tk.Button(head, text="Leeren", 
                                  command=ft.partial(self.execute_control_other,"clear"))
    ...
    def execute_control_command(self, action):
        self.data.execute_control_command(action, self)
        self.message.set(self.data.get_status())
        
        
    def execute_control_query(self, action):
        self.data.execute_control_query(action, self)
        self.message.set(self.data.get_status())


    def execute_control_other(self, action):
        self.data.execute_control_other(action, self)
        self.message.set(self.data.get_status())
