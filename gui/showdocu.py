...
class ShowDocu(win.Window):
    def __init__(self, parent, title, schema_name, table_name, display_mode, format_name):
        win.Window.__init__(self, parent, "ShowDoku", title, self.get_size(800,600,700,80))

        config = cnf.Config.get_instance()
        bgcolor = config.get("show.bg.color")
        print("ShowDocu.init: title=" + title + ",  schema=" + schema_name + 
        ...
        if format_name == "pdf":
            ...
            pdffilename, rows, cols = pd.process_docu(schema_name, table_name)
            self.geometry(self.get_size(100*(cols+1),50*(rows+8),600,100))
            ...
            htmlfilename, rows, cols = ht.process_docu(schema_name, table_name)
            self.geometry(self.get_size(100*(cols+1),40*(rows+8),600,100))
